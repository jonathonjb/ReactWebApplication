...
const KINGSIDE_CASTLE = 10;
const QUEENSIDE_CASTLE = 11;
...
const getEligiblePositions = (row, col, board, castlingCodes, advancedPawnPos) => {
...
const positionContainsUserPiece = (row, col, userIsWhite, board) => {
    return userIsWhite ? board[row][col] > 0 : board[row][col] < 0;
}

const getIfPinned = (row, col, isWhite, numOfchecks, board) => {
    let tempBoard = board.map(row => row.slice());
    tempBoard[row][col] = EMPTY;
    return getNumOfChecks(isWhite, tempBoard) > numOfchecks;
}

const getNumOfChecks = (isWhite, board) => {
...
const getKingPosition = (isWhite, board) => {
...
module.exports = {
    ...
    QUEEN: QUEEN,
    KING: KING,
    getEligiblePositions: getEligiblePositions,
    positionContainsPiece: positionContainsPiece,
    positionContainsUserPiece: positionContainsUserPiece,
    getIfPinned: getIfPinned,
    getNumOfChecks: getNumOfChecks
}